...
function compareToValueFile {
	...
	if [ $VAL -eq $1 ] ; then 
		echo -e `date` "[ZONE THREE]: no state change required in value: Current value -> $VAL" >> $CRON_LOG;
		exit 0;
	fi
}
...
if [ $EXECUTE -gt 0 ] ; then
	...
	echo "1" > $GPIO_VALUE;
	echo -e `date` "[ZONE THREE]:  GPIO3 turned ON: " >> $CRON_LOG;

else 
	compareToValueFile 0;
	echo "0" > $GPIO_VALUE;
	echo -e `date` "  [ZONE THREE]:  GPIO3 turned OFF."  >> $CRON_LOG;
fi
